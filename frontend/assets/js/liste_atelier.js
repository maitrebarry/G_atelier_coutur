...
async function editAtelier(id) {
    try {
        ...
        document.getElementById('editTelephoneAtelier').value = atelier.telephone || '';
        ...
        // Ouvrir le modal
        const editModal = new bootstrap.Modal(document.getElementById('editAtelierModal'));
        editModal.show();
...
document.getElementById('editAtelierForm').addEventListener('submit', async function(e) {
    e.preventDefault();
    ...
    const formData = {
        ...
        dateCreation: document.getElementById('editDateCreationAtelier').value || null
    };

    try {
        const response = await fetch(`http://localhost:8080/api/ateliers/${formData.id}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
        ...
        Swal.fire({
            ...
            // Fermer le modal
            const editModal = bootstrap.Modal.getInstance(document.getElementById('editAtelierModal'));
            editModal.hide();
...
document.getElementById('editAtelierModal').addEventListener('hidden.bs.modal', function () {
