...
const Permissions = () => {
  ...
  const handleUserSelect = async (user) => {
    // Allow PROPRIETAIRE to view/edit other PROPRIETAIRE permissions, but maybe warn or restrict SUPERADMIN
    ...
    try {
      ...
      let userPerms = res.data.map(p => p.id);

      // ✅ CORRECTION : Si c'est un PROPRIETAIRE ou SUPERADMIN et qu'il n'a aucune permission explicite,
      // on coche tout par défaut car ils ont un accès complet implicite.
      if ((user.role === 'PROPRIETAIRE' || user.role === 'SUPERADMIN') && userPerms.length === 0) {
          userPerms = permissions.map(p => p.id);
          // Optionnel : On pourrait afficher une notification pour dire que c'est des permissions par défaut
      }
